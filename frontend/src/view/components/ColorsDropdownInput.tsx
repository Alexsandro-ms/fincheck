...
export function ColorsDropdownInput({
    ...
    function handledSelectColor(color: Color) {
        ...
        <div>
            <DropdownMenu.Root>
                <DropdownMenu.Trigger>
                    <button
                        className={cn(
                            "w-full bg-white rounded-lg border outline-none border-gray-500 px-3 h-[52px] text-gray-700 focus:border-gray-800 transition-all text-left relative",
                ...
                <DropdownMenu.Content className="grid grid-cols-4">
                    {colors.map((color) => (
                        <DropdownMenu.Item
                            key={color.color}
                            onSelect={() => handledSelectColor(color)}
                        >
